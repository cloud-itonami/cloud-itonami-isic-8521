(ns secondary.operation
  "OperationActor -- one secondary-education operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (SchoolOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Curriculum Safeguarding
  ...
  One graph run = one secondary-education operation (intake -> advise
  ...
  `:grading/finalize`/`:graduation/finalize` ALWAYS reach this node
  when the governor is clean -- see `secondary.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [secondary.schoolopsllm :as schoolopsllm]
...
(defn build
  ...
  opts:
    :advisor      -- a `secondary.schoolopsllm/Advisor` (default: mock-advisor)
