(ns secondary.schoolopsllm
  "SchoolOps-LLM client -- the *contained intelligence node* for the
  secondary-education actor.

  It normalizes student-intake, drafts a per-jurisdiction secondary-
  education evidence checklist, screens students for an unresolved
  academic-integrity concern, drafts the grading-finalization action,
  and drafts the graduation-finalization action. CRITICAL: it is a
  smart-but-untrusted advisor. It returns a *proposal* (with a
  rationale + the fields it cited), never a committed record or a real
  grading/graduation finalization. Every output is censored downstream
  by `secondary.governor` before anything touches the SSoT, and
  `:grading/finalize`/`:graduation/finalize` proposals NEVER auto-
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :stake      kw|nil         ; :actuation/finalize-grading | :actuation/finalize-graduation | nil
     :confidence 0..1}"
...
(defn- assess-jurisdiction
  "Per-jurisdiction secondary-education evidence checklist draft.
  ...
  in `secondary.facts` -- the Curriculum Safeguarding Governor must
  ...
  (let [s (store/student db subject)
    ...
    (if (nil? sb)
      {:summary    (str iso3 " の公式spec-basisが見つかりません")
       :rationale  "secondary.facts に未登録の法域。要件を推測で作らない。"
       :cites      []
...
(defn- propose-grading-finalization
  "Draft the actual GRADING-FINALIZATION action -- finalizing a real
  student's course grades. ALWAYS `:stake :actuation/finalize-grading`
  -- this is a REAL-WORLD academic-record act, never a draft the actor
  may auto-run. See README `Actuation`: no phase ever adds this op to
  a phase's `:auto` set (`secondary.phase`); the governor also always
  escalates on `:actuation/finalize-grading`. Two independent layers
  agree, deliberately."
  [db {:keys [subject]}]
  (let [s (store/student db subject)]
    {:summary    (str subject " 向け成績確定提案"
     ...
     :rationale  (if s
                   ...
                   "生徒記録が見つかりません")
     :cites      (if s [subject] [])
     :effect     :student/mark-graded
     :value      {:student-id subject}
     :stake      :actuation/finalize-grading
     :confidence (if (and s (not (registry/attendance-hours-insufficient? s))) 0.9 0.3)}))
...
(defn- propose-graduation-finalization
  ...
  adds this op to a phase's `:auto` set (`secondary.phase`); the
...
(defn infer
  ...
  (case op
    ...
    :grading/finalize           (propose-grading-finalization db request)
...
(def ^:private system-prompt
  (str "あなたは中等学校の成績確定・卒業確定エージェントの助言者です。"
       "与えられた事実のみに基づき、提案を1つだけEDNマップで返します。説明や前置きは"
       "一切書かず、EDNだけを出力します。\n"
       "キー: :summary(人向けドラフト) :rationale(根拠/必ず事実から) "
       ":cites(使った事実キーのベクタ) "
       ":effect(:student/upsert|:assessment/set|:integrity-screen/set|"
       ":student/mark-graded|:student/mark-graduated) "
       ":stake(:actuation/finalize-grading か :actuation/finalize-graduation か nil) :confidence(0..1)。\n"
...
(defn- facts-for [st {:keys [op subject]}]
  (case op
    ...
    :grading/finalize           {:student (store/student st subject)}
...
(defn- parse-proposal
  ...
  grading or auto-finalize a graduation."
  [content]
